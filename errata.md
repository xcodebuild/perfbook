...
- 201 页，`IndexDB` 应当为 `IndexedDB`

- 166 页倒数第10行`……这意味着解析、执行 JavaScript、重新布局等依赖主线程完成的动画……` 应修改为 `……这意味着解析和执行 JavaScript、重新布局等依赖主线程完成的动画……`

- 176 页第 3 行“通过这种方式，JavaScript代码在单进程下运行” 这句话中的`进程`应该修正为`线程`
